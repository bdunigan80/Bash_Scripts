...
Usage: $0 [-o output] [-i|-f] [-n]
  ...
  -i is and incremental, -f is a full backup, and -n prevents
  updating the timestamp when an incremental backup is done.
EOF
  exit 1
}

############# Main code selection begins here #######################

while getopts "o:ifn" arg; do
  case "$opt" in
    o ) output="$OPTARG";		;;   # getopts automaticlly manages OPTARG
    i ) btype="incremental";		;;
    f ) btype="full";	         	;;
    n ) noinc=1;			;;
    ? ) usageQuit	                ;;
...
timestamp="$(date + '%m%d%I%M')"  # Grabe month, day, hour, minute from date.
  				   # Curious about date formats? "man strftime"

if [ "$btype" = "incremental" ] ; then
  if [ ! -f $tsfile ] ; then
    echo "Error: can't do and incremental backup: no timestamp file" >&2
    exit 1
  fi
